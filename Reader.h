...
void importTrainFiles() {
	...
	FILE *fin;
	int tmp;
	std::string fileName = inPath + "relation2id.txt";
	fopen_s(&fin,fileName.c_str(), "r");
	tmp = fscanf_s(fin, "%ld", &relationTotal);
	printf("The total of relations is %ld.\n", relationTotal);
	fclose(fin);

	fopen_s(&fin, (inPath + "entity2id.txt").c_str(), "r");
	tmp = fscanf_s(fin, "%ld", &entityTotal);
	printf("The total of entities is %ld.\n", entityTotal);
	fclose(fin);

	fopen_s(&fin, (inPath + "train2id.txt").c_str(), "r");
	tmp = fscanf_s(fin, "%ld", &trainTotal);
	...
	for (INT i = 0; i < trainTotal; i++) {
		tmp = fscanf_s(fin, "%ld", &trainList[i].h);
		tmp = fscanf_s(fin, "%ld", &trainList[i].t);
		tmp = fscanf_s(fin, "%ld", &trainList[i].r);
	}
	fclose(fin);
...
void importTestFiles() {
    ...
    fopen_s(&fin, (inPath + "relation2id.txt").c_str(), "r");
    tmp = fscanf_s(fin, "%ld", &relationTotal);
    fclose(fin);
	...
	FILE* f_kb1;
	fopen_s(&f_kb1, (inPath + "test2id.txt").c_str(), "r");
	FILE* f_kb2;
	fopen_s(&f_kb2, (inPath + "train2id.txt").c_str(), "r");
	FILE* f_kb3;
	fopen_s(&f_kb3, (inPath + "valid2id.txt").c_str(), "r");
    tmp = fscanf_s(f_kb1, "%ld", &testTotal);
    tmp = fscanf_s(f_kb2, "%ld", &trainTotal);
    tmp = fscanf_s(f_kb3, "%ld", &validTotal);
    ...
    for (INT i = 0; i < testTotal; i++) {
        tmp = fscanf_s(f_kb1, "%ld", &testList[i].h);
        tmp = fscanf_s(f_kb1, "%ld", &testList[i].t);
        tmp = fscanf_s(f_kb1, "%ld", &testList[i].r);
        tripleList[i] = testList[i];
    }
    for (INT i = 0; i < trainTotal; i++) {
        tmp = fscanf_s(f_kb2, "%ld", &tripleList[i + testTotal].h);
        tmp = fscanf_s(f_kb2, "%ld", &tripleList[i + testTotal].t);
        tmp = fscanf_s(f_kb2, "%ld", &tripleList[i + testTotal].r);
    }
    for (INT i = 0; i < validTotal; i++) {
		tmp = fscanf_s(f_kb3, "%ld", &tripleList[i + testTotal + trainTotal].h);
		tmp = fscanf_s(f_kb3, "%ld", &tripleList[i + testTotal + trainTotal].t);
		tmp = fscanf_s(f_kb3, "%ld", &tripleList[i + testTotal + trainTotal].r);
